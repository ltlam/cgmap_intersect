...
def sort_cgmap(str_cgmap,str_temp_cgmap_folder,str_cntxt,n_min_cov,n_sorted):
    b_is_CGmap = str_cgmap.endswith('.CGmap')
    ...
    if n_sorted:
        ...
        bed_df = pd.read_table(str_cgmap,sep='\t',header=None)
        sorted_df = bed_df.sort_values(by=[0,1],ascending=[1,1])
        sorted_df.to_csv(str_sorted_file,na_rep='nan',sep='\t',header=False,
                         index=False)

#------------------------------------------------------------------------------
# get_meth_info - extract coordinate and methylation values from list of str
# [in] b_iscgmap - integer flag 1, if CGmap, 0 assumes bed graph
# [in] l_str_vals -
# [out] (str_chr,n_pos,f_meth)
#------------------------------------------------------------------------------
def get_meth_info(b_iscgmap,l_str_vals):
    str_chr = l_str_vals[0]
    if b_iscgmap:
        n_pos = int(l_str_vals[2])
        f_meth = float(l_str_vals[5])
    else:
        n_pos = (int(l_str_vals[1])+int(l_str_vals[2]))/2
        f_meth = float(l_str_vals[3])
    return (str_chr,n_pos,f_meth)
...
def write_intersection(l_sorted_cgmap,str_out):
    ...
    if len(l_cur_chr) == 0:
        for n_fid in range(len(l_fopen)):
            ...
            str_chr, n_pos, f_meth = get_meth_info(l_is_CGmap[n_fid],lcols)
            l_cur_chr.append(str_chr)
            l_cur_pos.append(n_pos)
            l_cur_meth.append(f_meth)
    ...
    while(l_cur_chr.count(STR_MAX_CHR) != len(l_sorted_cgmap)):
        ...
        for n_min_id in l_min_pos_id:
            ...
            if str_file_line != '':
                lcols = str_file_line.split('\t')
                str_chr, n_pos, f_meth = get_meth_info(l_is_CGmap[n_min_id],lcols)
                l_cur_chr[n_min_id] = str_chr
                l_cur_pos[n_min_id] = n_pos
                l_cur_meth[n_min_id] = f_meth
            else:
...
def get_files_to_sam_lbl(str_cgmap_files):
    ...
    for str_cgmap in l_cgmap_files:
        head,tail = os.path.split(str_cgmap)
        l_sams.append(re.sub('.CGmap$|.txt$|.bed$','',tail))
    return ','.join(l_sams)
